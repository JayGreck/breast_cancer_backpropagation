...
class Dense_Layer:
    ...
    def update_weights(self, layer, neural_net, learning_rate):
        for i in range(len(neural_net)):
            ...
            for neuron in neural_net[i]:
                for z in range(len(neuronal_inputs)):
                    neuron['weights'][z] -= np.multiply(learning_rate, neuron['error_signal'], neuronal_inputs[z])
                neuron['weights'][-1] -= np.multiply(learning_rate, neuron['error_signal'])
